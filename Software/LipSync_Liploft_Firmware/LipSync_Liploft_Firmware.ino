...
// NEED THIS SECTION FOR THE 1ST TIME OF CONFIGURING THE BT MODULE
//  int val0 = 0;
//  EEPROM.put(0,val0);
//  
//  delay(2000);
//  while(!Serial1); 
  ...
  if(DEBUG_MODE==true) {Serial.println("Beginning initialization...");}
...
//      if (!rawBluetoothMode) { 
//      Serial.print("Lipsync_mouth");
//    } else {
//      Serial.print("Lipsync_motormouth");
//    }
  }
}
...
void bluetoothConfigure(void) {
  ...
  if (bluetoothConfigDone == 0 || BT_CONFIG_FLAG) {        //If Bluetooth has not been configured or Bluetooth config flag is true then execute configuration sequence
    ...
    if (!rawBluetoothMode) {
      ...
      Serial.println("LipSync_mouse"); //Send configuarion data to Bluetooth module
    } else {
      Serial.println("LipSync_motormouth");
...
void bluetoothConfigSequenceForRawTx(void) {
  ...
  Serial1.println("SX,0");                          //Set Bonding to 0 or disabled
  delay(15);
  Serial1.println("SN,LipSync_SHAWN"); //("SN,LipSync_MOTORMOUTH");        //Set the name of BT module
  delay(15);
